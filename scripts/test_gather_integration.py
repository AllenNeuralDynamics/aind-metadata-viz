...
def main():
    ...
    print(f"📄 Using instrument from metadata.json: {instrument['object_type'] if instrument else 'None'}")
    ...
    try:
        ...
        if validation_response.status_code == 200:
            ...
            print(f"❌ Combined metadata validation failed: {validation_response.text}")
    ...
    for component_name, component_data in components_to_test:
        try:
            ...
            if component_response.status_code == 200:
                ...
                print(f"❌ {component_name} validation failed: {component_response.text}")
    ...
    print(f"   - Acquisition: {'✅ (from file)' if acquisition else '❌ (missing)'}")
    print(f"   - Instrument: {'✅ (from file)' if instrument else '❌ (missing)'}")
